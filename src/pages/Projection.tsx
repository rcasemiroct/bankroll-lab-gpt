...
import { IconAlertTriangle, IconCalendarStats, IconInfoCircle, IconTargetArrow } from "@tabler/icons-react";
...
export function Projection({ data }: { data: ReturnTypeOfUseAppData }) {
  ...
  return <>
    ...
    <div className="flex flex-col gap-3">
      <Alert className="border-warning/50 bg-warning/5"><IconAlertTriangle className="mb-2 text-warning" /><AlertTitle className="text-warning">Projeção não é previsão.</AlertTitle><AlertDescription>Ela depende de vantagem real, disciplina no valor das apostas e amostra estatística suficiente.</AlertDescription></Alert>
      <Card><CardHeader><CardTitle>Premissas</CardTitle><CardDescription>Ajuste os parâmetros para comparar cenários</CardDescription></CardHeader><CardContent><FieldGroup><div className="grid grid-cols-2 gap-3"><Field><FieldLabel>Banca inicial</FieldLabel><Input type="number" inputMode="decimal" value={initial} onChange={(event) => setInitial(Number(event.target.value))} /></Field><Field><FieldLabel>Meta final</FieldLabel><Input type="number" inputMode="decimal" value={target} onChange={(event) => setTarget(Number(event.target.value))} /></Field></div><div className="grid grid-cols-2 gap-3"><Field><FieldLabel>Crescimento por ciclo (%)</FieldLabel><Input type="number" inputMode="decimal" step="0.1" value={expectedReturn} onChange={(event) => setExpectedReturn(Number(event.target.value))} /></Field><Field><FieldLabel>Ciclos por semana</FieldLabel><Input type="number" min="1" value={cyclesPerWeek} onChange={(event) => setCyclesPerWeek(Number(event.target.value))} /></Field></div><div className="grid grid-cols-[auto_1fr] gap-2 rounded-md border bg-secondary/60 p-3"><IconInfoCircle className="mt-0.5 size-4 text-primary" /><div><strong className="block text-xs">O que é um ciclo?</strong><p className="mb-0 mt-1 text-xs leading-relaxed text-muted-foreground">É uma repetição do seu plano, como uma sessão ou um dia de apostas. Informar 2% significa multiplicar a banca por 1,02 a cada ciclo; “ciclos por semana” transforma isso em calendário. Para sortear vitórias e perdas aposta por aposta, use a tela Simulação.</p></div></div><Field><FieldLabel>Data inicial</FieldLabel><Input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></Field></FieldGroup></CardContent></Card>
