  ...
  return <>
    ...
    <div className="flex flex-col gap-3">
      <Card><CardContent className="grid grid-cols-[1fr_1fr_.92fr] gap-0 p-3">
        <MetricBlock label="Banca atual" value={currency.format(metrics.activeBankroll)} />
...
function MetricBlock({ label, value, tone, className = "" }: { label: string; value: string; tone?: "positive" | "negative"; className?: string }) { return <div className={`flex min-w-0 flex-col gap-2 ${className}`}><span className="min-h-5 text-[8px] font-semibold uppercase leading-tight tracking-wider text-muted-foreground">{label}</span><strong className={`tabular truncate text-sm sm:text-2xl ${tone === "positive" ? "text-positive" : tone === "negative" ? "text-destructive" : ""}`}>{value}</strong></div>; }
