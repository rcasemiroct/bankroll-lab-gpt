...
export function Bets({ data }: { data: ReturnTypeOfUseAppData }) {
  ...
  const strategyProfit = strategyMetrics.map((item) => ({ name: item.strategy, value: item.profit }));
  const strategyRoi = strategyMetrics.map((item) => ({ name: item.strategy, value: item.roi * 100 }));
  const houseProfit = [...data.bets.filter((bet) => bet.status !== "pending").reduce((groups, bet) => groups.set(bet.sportsbook || "Sem casa", (groups.get(bet.sportsbook || "Sem casa") ?? 0) + bet.profit), new Map<string, number>())].map(([name, value]) => ({ name, value }));
  ...
  return <>
    ...
    <div className="flex flex-col gap-3">
      ...
      {strategyMetrics.length > 0 && <div className="grid gap-3 sm:grid-cols-2"><Card><CardHeader><CardTitle>Resultado por estratégia</CardTitle><CardDescription>Lucro ou perda acumulada</CardDescription></CardHeader><CardContent><BreakdownChart data={strategyProfit} /></CardContent></Card><Card><CardHeader><CardTitle>ROI por estratégia</CardTitle><CardDescription>Retorno sobre o valor apostado</CardDescription></CardHeader><CardContent><BreakdownChart data={strategyRoi} valueType="percent" /></CardContent></Card></div>}
      {houseProfit.length > 0 && <Card><CardHeader><CardTitle>Resultado por casa</CardTitle><CardDescription>Distribuição do resultado liquidado</CardDescription></CardHeader><CardContent><BreakdownChart data={houseProfit} /></CardContent></Card>}
