    ...
    <DrawerContent>
      ...
      <div className="overflow-y-auto px-5 pb-5 scrollbar-none">
        <FieldGroup>
          ...
          <div className="grid grid-cols-2 gap-3"><Field><FieldLabel htmlFor="bet-odds">Odd decimal</FieldLabel><Input id="bet-odds" type="number" inputMode="decimal" min="1.01" step="0.01" value={draft.odds} onChange={(event) => setDraft({ ...draft, odds: Number(event.target.value) })} /></Field><Field><FieldLabel htmlFor="bet-stake">Stake (R$)</FieldLabel><Input id="bet-stake" type="number" inputMode="decimal" min="0" step="0.01" value={draft.stake} onChange={(event) => setDraft({ ...draft, stake: Number(event.target.value) })} /><FieldDescription className={stakePercentage > rules.strongAlertPercentage ? "text-destructive" : stakePercentage > rules.maxStakePercentage ? "text-warning" : undefined}>{(stakePercentage * 100).toFixed(1)}% da banca • limite {(rules.maxStakePercentage * 100).toFixed(1)}%</FieldDescription></Field></div>
