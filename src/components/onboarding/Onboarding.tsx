...
export function Onboarding() {
  ...
  const steps = [
    ...
    <><h2 className="text-2xl font-semibold">Defina os limites</h2><FieldGroup><Field><FieldLabel>Banca inicial</FieldLabel><Input type="number" inputMode="decimal" value={initialBankroll} onChange={(event) => setInitialBankroll(Number(event.target.value))} /></Field><Field><FieldLabel>Meta final</FieldLabel><Input type="number" inputMode="decimal" value={target} onChange={(event) => setTarget(Number(event.target.value))} /></Field><div className="grid grid-cols-2 gap-3"><Field><FieldLabel>Stop diário (%)</FieldLabel><Input type="number" value={dailyStop} onChange={(event) => setDailyStop(Number(event.target.value))} /></Field><Field><FieldLabel>Valor máximo por aposta (%)</FieldLabel><Input type="number" value={maxStake} onChange={(event) => setMaxStake(Number(event.target.value))} /></Field></div></FieldGroup></>,
